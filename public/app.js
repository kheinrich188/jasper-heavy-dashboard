...
const chartPanelEls = Array.from(document.querySelectorAll(".chart-panel"));
...
function ensureLoadingCards() {
  cardsEl.innerHTML = Array.from({ length: 4 })
    .map(
      () => `
      <article class="kpi-card loading-card">
        <span class="skeleton-line skeleton-title"></span>
...
function setLoadingState(isLoading) {
  ...
  chartPanelEls.forEach((panel) => panel.classList.toggle("panel-loading", isLoading));
  if (isLoading) {
    ensureLoadingHero();
    ensureLoadingCards();
  ...
  heroEl.innerHTML = `
    <div class="hero-layout">
      ...
      <div class="hero-right">
        ...
        <div class="hero-inactive">Inaktiv seit ${formatDuration(inactivitySeconds)}</div>
      </div>
    </div>
  `;
        ...
        .map((s) => {
          ...
          return `
            <article class="session-item">
              ...
              <div class="session-metric">Dauer ${formatDuration(s.durationS)}</div>
              <div class="session-metric">Ø ${fmt(avgKmh, " km/h")} · Max ${fmt(s.maxKmh, " km/h")}</div>
